...
- **Transport failures become `LLMError(network)`** — a `ClientException` from an unreachable endpoint previously escaped the provider, and `FallbackProvider` only catches `LLMError`, so a configured `fallback_model` was never tried during the network outage it exists to cover. Both request paths and the mid-stream body now convert transport errors
- **Context window derived per model** — the provider advertised a flat 128K for every model, but `ContextBuilder` derives the whole token budget (including a 10% output allowance) from it, so an 8K model served over a compatible endpoint would start failing as the session grew. Known OpenAI families are matched by prefix; anything unrecognised gets a conservative 8K, overridable with `openai_context_window`
- **Provider-aware auth hints** — an auth failure on an OpenAI model told the user to set `ANTHROPIC_API_KEY`; the hint now names the active provider's variable
